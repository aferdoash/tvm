...
def verify_pool3d_orig(n, ic, ih, kh, sh, padding, pool_type, ceil_mode, count_include_pad=True):
    id = iw = ih
    kd = kw = kh
    sd = sw = sh
    pbk, pt, pl, pfw, pb, pr = padding
    layout = "NCDHW"
    A = tvm.placeholder((n, ic, id, ih, iw), name='A')
    B = topi.nn.pool(A, kernel=[kd, kh, kw], stride=[sd, sh, sw], padding=padding,
                     pool_type=pool_type, ceil_mode=ceil_mode,
                     layout="NCDHW", count_include_pad=count_include_pad)
    B = topi.nn.relu(B)
    dtype = A.dtype
    ...
    ashape = get_const_tuple(A.shape)
    if ceil_mode:
        assert bshape[2] == int(math.ceil(float(ashape[2] - kd + pbk + pfw) / sd) + 1)
        assert bshape[3] == int(math.ceil(float(ashape[3] - kh + pt + pb) / sh) + 1)
        assert bshape[4] == int(math.ceil(float(ashape[4] - kw + pl + pr) / sw) + 1)
    else:
        assert bshape[2] == int(math.floor(float(ashape[2] - kd + pbk + pfw) / sd) + 1)
        assert bshape[3] == int(math.floor(float(ashape[3] - kh + pt + pb) / sh) + 1)
        assert bshape[4] == int(math.floor(float(ashape[4] - kw + pl + pr) / sw) + 1)

    a_np = np.random.uniform(low=0.001, size=(n, ic, id, ih, iw)).astype(dtype)
    pad_np = np.zeros(shape=(n, ic, id+pbk+pfw, ih+pt+pb, iw+pl+pr)).astype(dtype)
    no_zero = (range(n), range(ic), (range(pbk, id+pbk)), (range(pt, ih+pt)), (range(pl, iw+pl)))
    pad_np[np.ix_(*no_zero)] = a_np
    _, oc, od, oh, ow = get_const_tuple(B.shape)
    b_np = np.zeros(shape=(n, oc, od, oh, ow)).astype(dtype)
